...
public record PersonDTO(
    ...
    public PersonDTO(PersonEntity p) {
        this(p.getId() == null ? new ObjectId().toHexString() : p.getId().toHexString(), p.getFirstName(),
             p.getLastName(), p.getAge(), p.getEmail(), new AddressDTO(p.getAddress()), p.getCreatedAt(), p.getInsurance(),
             p.getCars().stream().map(CarDTO::new).toList());
    }

    public PersonEntity toPersonEntity() {
        ObjectId _id = id == null ? new ObjectId() : new ObjectId(id);
        return new PersonEntity(_id, firstName, lastName, age, email, address.toAddressEntity(), createdAt, insurance,
